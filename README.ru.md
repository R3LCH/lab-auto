...
- 📥 **Загрузки** — `task.pdf` (задание); `reports/site-report-<id>.pdf` при первом sync, если отчёт уже есть на GUAP
...
`sync` переименовывает папки при смене статуса. Импорт отчёта с сайта — один раз на работу, если `reports/` пуст и на GUAP `ожидает проверки` или `принят`.
