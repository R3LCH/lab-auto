...
from lab_auto.materials import local_material_path
from lab_auto.paths import build_work_id, legacy_work_id, rel_path

if TYPE_CHECKING:
    from lab_auto.subject_materials import SubjectMaterial
...
def _represent_state_string(dumper: yaml.SafeDumper, value: str) -> yaml.ScalarNode:
    return dumper.represent_scalar("tag:yaml.org,2002:str", value, style="|" if "\n" in value else None)


_StateDumper.add_representer(str, _represent_state_string)
...
def _save_records_unlocked(root: Path, works: list[WorkRecord]) -> None:
    ...
    content = yaml.dump(payload, Dumper=_StateDumper, allow_unicode=True, sort_keys=False)
...
def generate_task_markdown(
    work: WorkRecord,
    subject_materials: list[SubjectMaterial] | None = None,
    *,
    root: Path | None = None,
) -> None:
    """Write the generated entry point from the final, merged sync record."""
    attachment = "[task.pdf](task.pdf)" if (work.folder / "task.pdf").is_file() else "Нет"
    material = "Нет"
    if work.additional_material_url:
        url = work.additional_material_url
        for char, escaped in ((" ", "%20"), ("(", "%28"), (")", "%29"), ("<", "%3C"), (">", "%3E"), ("\n", "%0A"), ("\r", "%0D")):
            url = url.replace(char, escaped)
        material = f"[Открыть внешний материал]({url})"
    local = local_material_path(work.folder, work.additional_material)
    if local:
        name = local.name.replace("[", "\\[").replace("]", "\\]")
        source = quote(
            work.additional_material.source_url or work.additional_material_url or "",
            safe=":/?=&%#@+~,;!$",
        )
        material_link = f"[{name}]({quote(work.additional_material.path)})\n\nИсточник: [ссылка]({source})"
        if work.additional_material_url and work.additional_material_url != work.additional_material.source_url:
            material = material_link + "\n\nАктуальная ссылка: " + material
        else:
            material = material_link
    sections = [
        "<!-- Generated by lab-auto. Local edits may be overwritten. -->",
        f"# {markdown_text(work.name)}",
        "## Дисциплина", markdown_text(work.subject),
        "## ID задания", markdown_text(work.work_id),
        "## Дедлайн", markdown_text(work.due_date or "Не указан"),
        "## Статус", work.local_status.prefix,
        "## Описание задания",
        work.description or "Описание задания на странице не указано.",
    ...
    if subject_materials is not None and root is not None:
        ...
        sections.extend([
            ...
            task_material_markdown(root, work, subject_materials),
        ])
    content = "\n\n".join(sections) + "\n"
    destination = work.folder / "task.md"
